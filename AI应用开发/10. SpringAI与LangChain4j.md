...
> 「Java 转 AI」的差异化必考点：面试官要验证你会不会**写代码**而不只画架构图。本文把 [RAG](3. RAG检索增强生成.md)、[Agent](7. Agent智能体框架.md)、[安全](13. LLM安全与工程化.md) 的思想落到 Java 框架层——版本基线：**Spring AI 1.1 GA**（2025-11，MCP 原生支持；2.x 已发布但企业采用以 1.1 为主）、**LangChain4j 1.x**（2026-08 已到 1.19）。
...
|:---|:---|:---|
| `MessageChatMemoryAdvisor` | 注入对话历史（记忆） | [Agent 记忆三层](7. Agent智能体框架.md) |
| `QuestionAnswerAdvisor` | 检索知识库 + 拼上下文（**一行接 RAG**） | [RAG 检索增强生成](3. RAG检索增强生成.md) |
| `SimpleLoggerAdvisor` | 请求/响应日志 | 可观测 |
| `ToolCallAdvisor`（1.1） | **工具调用循环进链**：禁用模型内部执行，链上其他 Advisor 能观察每一轮工具调用 | [Agent 智能体框架](7. Agent智能体框架.md) |
...
> 1.1 的 **Recursive Advisors**：Advisor 可以多次调用下游子链（`chain.copy(after)`）——工具循环、输出校验重试、LLM-as-Judge 迭代都能做成链上组件。与 [双引擎评测](13. LLM安全与工程化.md) 的「校验不达标重试」直接同构。
...
对应站内体系：`EmbeddingModel`（选型见 [大模型工程基础](1. 大模型工程基础.md)）、切片策略、混合检索在 Spring AI 里通过自定义 Advisor/`VectorStore` filter 组合落地。
...
对应站内：多 Agent 编排思想见 [Agent 智能体框架](7. Agent智能体框架.md)（区别：LangChain4j 的编排仍是模型自主性较强，生产强管控场景你仍会选自研状态机——这就是「框架 vs 自研」的选型答案）。
...
- 抽象粒度：**RetrievalAugmentor 可自由组合改写/路由/聚合/重排**——把 [双混合检索架构](3. RAG检索增强生成.md) 用组件拼出来
...
| 怎么防止工具死循环烧钱？ | 工具调用次数上限 + 单轮 token 限额 + 总预算熔断（对齐 [成本优化](14. AI高频面试题速查.md) T28） |
| entity() 结构化输出可靠吗？ | 不可靠时是常态：schema 注入提高成功率 + 解析失败带错误重试 + 关键字段校验（对齐速查 T21） |
| AiServices 怎么实现的？ | JDK 动态代理翻译接口调用（与 MyBatis Mapper 同构） |
| 流式场景注意什么？ | Advisor 分同步/流式两接口；SSE 转发注意粘包半包（[LLM 安全与工程化 §五](13. LLM安全与工程化.md)） |
...
| 拿框架硬套强管控 Agent | 生产变更类链路仍需过程式状态机兜底（框架循环是模型自主的），参考 [Agent 框架篇选型](7. Agent智能体框架.md) |
...
- 三态 Hook ↔ Advisor/Guardrails：[LLM 安全与工程化](13. LLM安全与工程化.md)
- RAG 组件与检索原理：[RAG 检索增强生成](3. RAG检索增强生成.md)
- 工具循环与 Agent 编排：[Agent 智能体框架](7. Agent智能体框架.md)
