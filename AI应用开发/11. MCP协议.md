...
> 本文是 [AI高频面试题速查](14. AI高频面试题速查.md) T14 的**详细篇**，与 [Agent智能体框架](7. Agent智能体框架.md)（工具是 Agent 的手脚）、[SpringAI与LangChain4j](10. SpringAI与LangChain4j.md)（框架层落地）互为犄角。版本口径与站内基线一致：**Spring AI 1.1 GA 原生支持、Streamable HTTP 为推荐传输（SSE 已废弃）**。
...
版本基线：**Spring AI 1.1 GA（2025-11）MCP 原生支持**——注解式暴露 + 客户端/服务端 Starter（详见 [SpringAI与LangChain4j](10. SpringAI与LangChain4j.md) §2.5）。
...
| **混淆代理问题** | 受信 Server 的数据里夹带注入指令，借模型之手执行 | 工具输出当**数据**不当**指令**（与 RAG 注入防御同源，见 [LLM安全与工程化](13. LLM安全与工程化.md)） |
...
5. **工具太多模型选不过来怎么办？** → 按场景分组/检索式工具选择（先 tools/search 再 call），别一次塞几十个 schema（与 [Agent智能体框架](7. Agent智能体框架.md) 上下文预算一致）。
...
- 速查骨架：[AI高频面试题速查](14. AI高频面试题速查.md) T14
- 上层框架：[Agent智能体框架](7. Agent智能体框架.md)（ReAct ⚔️ 状态机、工具循环、发散治理）
- 框架落地：[SpringAI与LangChain4j](10. SpringAI与LangChain4j.md)（@McpTool、Starter、传输选型）
- 安全纵深：[LLM安全与工程化](13. LLM安全与工程化.md)、[沙箱机制](9. 沙箱机制.md)
