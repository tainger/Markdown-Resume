...
> 已有 [3. RAG检索增强生成.md](3. RAG检索增强生成.md) 聚焦**检索策略层**（混合检索/切片/重排），本篇聚焦**集成架构层**——检索到的内容怎么注入 Prompt、怎么管理 Token 预算、怎么做流式调用、怎么处理异常降级、Spring AI/LangChain4j 怎么封装。
...
│      │                                                           │
│      ▼                                                           │
│  [2] 检索（Retrieval）—— 详见 3. RAG检索增强生成.md                  │
│      ├─ 向量检索 + 关键词检索（双路）                            │
│      ├─ Cross-Encoder 重排                                       │
│      └─ 回调父级章节补全上下文                                    │
│      │                                                           │
...
- 检索策略层（混合检索/切片/重排）→ [3. RAG检索增强生成.md](3. RAG检索增强生成.md)
- Spring AI 框架细节（Advisor 链/ChatClient）→ [10. SpringAI与LangChain4j.md](10. SpringAI与LangChain4j.md)
- 幻觉治理（RAG 集成的核心目标）→ [6. 幻觉，幻觉审计，幻觉治理，幻觉评测.md](6. 幻觉，幻觉审计，幻觉治理，幻觉评测.md)
- Prompt 工程（System Prompt 设计）→ [2. 提示词工程.md](2. 提示词工程.md)
- LLM 安全（pre/do/post Hook 后处理）→ [13. LLM安全与工程化.md](13. LLM安全与工程化.md)
- SSE 流式输出（AgentMate 实战）→ [13. LLM安全与工程化.md](13. LLM安全与工程化.md) SSE 章节
