...
我们在项目中用的比较多的是这个基于 Word 文档中标题做分片的分片器。主要的功能和 Markdown 标题分片差不多，但是还有一些差异。

首先，Markdown 的标题是有明确语法的，比如 `#`、`##`、`###` 这种，而 Word 不一定。虽然有些文档中的标题会采用特殊格式，如：

![父子分片流程](./assets/11-parent-child-1.png)

这种的话，比较简单，我们可以通过 POI 解析出来。但是实际上，大多数文档其实是没有这么标准的，就需要有另外一套兜底的处理逻辑。

还有就是，有的时候，Word 文档内容可能会比较长，而 Embedding 模型都是有 Token 数限制的，如：

![父子分片实现](./assets/11-parent-child-2.png)

所以，如果只按照标题分的话，可能会比较长，所以还需要额外支持 chunkSize 和 overlap 的分片，即一个标题下，如果过长，则需要二次切分。（当然，这个可能在 Markdown 的分片器可能也是需要的，大家也可以自行实现）
