...
- 评测脚本 eval.py 准备与运行
  - 评测模型配置
    - AsyncOpenAI 客户端
    - llm_factory("qwen3.6-plus")：temperature=0、seed=42
    - enable_thinking=False 关思考模式，max_tokens=10240
    - embedding_factory("openai", text-embedding-v4)
  - 五大指标评分器
    - ContextPrecision / ContextRecall
    - AnswerRelevancy（需 embeddings）
    - Faithfulness
    - AnswerCorrectness（需 embeddings）
  - 评测流程
    - 读 generated_dataset.csv
    - contexts 按双换行切分并清洗
    - 单行5个 ascore 并发，asyncio.gather 等待
    - 全量并发 → 过滤None → 写 evaluation_results.csv
    - uv run eval.py 一键运行
  - 常见问题：max_tokens 截断
    - 报错 output is incomplete due to max_tokens
    - 解法1：关思考模式，reasoning_tokens 归0
    - 解法2：调高 max_tokens=10240
```
