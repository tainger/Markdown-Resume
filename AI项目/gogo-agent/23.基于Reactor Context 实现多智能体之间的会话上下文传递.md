...
```
Mono.just("Hello")    // 1. 上游：读取 Context    .flatMap(word -> Mono.deferContextual(ctx ->         Mono.just(word + ", " + ctx.get("user"))    ))    // 2. 下游：写入 Context（必须放在读取操作的下方）    .contextWrite(ctx -> ctx.put("user", "Alice"));
```
...
```
Mono<Msg> m = agent.call(msgs)        .contextWrite(Context.of("k", "A"));   // 内层，靠近数据源﻿m.contextWrite(Context.of("k", "B"))            // 外层，靠近 subscribe .subscribe();
```
...
| `ChatAgentExecutor:258` | `resume`恢复 ask\_user 暂停的 Agent |
...
![](https://cdn.nlark.com/yuque/0/2026/png/5378072/1786108597966-359056f1-a190-4cf5-85bf-9f18713f08a6.png)
...
```
﻿// ReActAgent:487，位于 model.stream(...).doOnNext(...) 内部notifyReasoningChunk(msg, context).subscribe();﻿// ReActAgent:580toolkit.setInternalChunkCallback(        (toolUse, chunk) -> notifyActingChunk(toolUse, chunk).subscribe());﻿// ReActAgent:770notifySummaryChunk(streamedMessage, ctx, generateOptions).subscribe();﻿
```
...
[✅基于Reactor Context 实现多智能体之间的会话上下文传递](https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a758d4d51b14400010724fa#slate-title)

[Reactor Context 的原理](https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a758d4d51b14400010724fa#6a758f42920ca6a1c6c6dddc)

[写入和读取方式](https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a758d4d51b14400010724fa#6a759055920ca64032c6de2f)

[为什么能在「主智能体 ↔ 子智能体」之间传递](https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a758d4d51b14400010724fa#6a7590b9920ca63739c6de4a)

[为什么能在「智能体 ↔ Hook」之间传递](https://thoughts.aliyun.com/workspaces/6963289eb0fc2e001bb052eb/docs/6a758d4d51b14400010724fa#6a759145920ca626a0c6de68)
