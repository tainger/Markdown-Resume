...
> **定位**：与 [SQL优化与调优](8.%20SQL优化与调优.md) 分工——那篇是「手段层」入口（explain 4 字段 + 慢日志 + 改写手法），这篇是「**字段层**」深度：12 个字段逐个解读 + 多表 JOIN 执行顺序 + key_len 计算公式 + Extra 完整速查 + optimizer_trace 看优化器内部决策。面试官追问字段细节就看这篇。
...
> [SQL优化与调优](8.%20SQL优化与调优.md) 讲过 type 排序、key、rows、Extra 4 个核心字段，本文重点补其余 8 个 + 4 个核心的深度细节。
...
> **优化目标**：至少到 **range**，理想到 **ref / const / eq_ref**。看到 `type=ALL` 且 rows 很大，重点优化（详见 [SQL优化的实际场景](11.%20SQL优化的实际场景.md) Q2）。
...
- 是优化器选索引的核心依据：rows 小的索引优先（[SQL优化的实际场景](11.%20SQL优化的实际场景.md) Q3「时快时慢」根因之一）
...
| `Using sort_union(...)` | 索引合并 OR，但需排序 | 中慢 | 看 [SQL优化的实际场景](11.%20SQL优化的实际场景.md) Q2 |

**最常见的危险信号 = `Using filesort` + `Using temporary`**，详见 [SQL优化与调优](8.%20SQL优化与调优.md) 第二节。
...
| **忘记 analyze 导致计划漂移** | 数据量增长/数据分布变化后统计信息过期，rows 估算失真 → 计划换索引变慢（详见 [SQL优化的实际场景](11.%20SQL优化的实际场景.md) Q3） |
...
| 主题 | 笔记 |
|:---|:---|
| explain 入门 4 字段 + 慢日志 + SQL 改写常见手法 | [SQL优化与调优.md](8.%20SQL优化与调优.md) |
| SQL 优化场景题（排查五步骨架、索引失效、时快时慢分诊、慢 SQL 治理） | [SQL优化的实际场景.md](11.%20SQL优化的实际场景.md) |
| 索引失效原理（B+ 树最左前缀、函数运算、隐式转换） | [索引.md](4.%20索引.md) |
| 深分页优化（key_len + 范围扫描在深分页里的代价） | [深分页优化.md](9.%20深分页优化.md) |
| JOIN 放大与笛卡尔积（漏 ON 的静默笛卡尔积） | [笛卡尔积.md](10.%20笛卡尔积.md) |
| SQL 执行底层全链路（EXPLAIN 对应优化器输出） | [15.一个sql执行底层mysql发生了哪些事情?.md](15.一个sql执行底层mysql发生了哪些事情%3F.md) |
| MySQL 8.0 Hash Join / 优化器新特性 | [存储引擎与架构.md](3.%20存储引擎与架构.md) |
