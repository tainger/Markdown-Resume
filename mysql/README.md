...
按主题整理的 MySQL 高频面试题，覆盖索引、事务与锁、锁机制深度、存储引擎与架构、SQL 优化、explain 深度、深分页、JOIN 笛卡尔积、分库分表、多租户、数据类型、MySQL 8.0 新特性、实际场景排查、SQL 执行底层全链路十四大板块。

## 目录

> 按由浅入深的学习路径编号：总览 → 基础 → 核心 → 调优 → 架构扩展 → 前沿

| # | 主题 | 笔记 | 核心考点 |
|:---|:---|:---|:---|
| 1 | 🗺️ 开篇词（知识脑图） | [1. 开篇词.md](1.%20开篇词.md) | MySQL 知识树总览，Mermaid mindmap 可交互脑图 |
| 2 | 🧮 数据类型选型 | [2. 数据类型选型.md](2.%20数据类型选型.md) | 整数/字符串/文本/浮点/时间/JSON/ENUM/字符集/主键选型、NULL 代价、隐式类型转换、2038 问题、UUID 性能 |
| 3 | ⚙️ 存储引擎与架构 | [3. 存储引擎与架构.md](3.%20存储引擎与架构.md) | InnoDB vs MyISAM、redo/undo/binlog、WAL、两阶段提交、Buffer Pool |
| 4 | 🔍 索引 | [4. 索引.md](4.%20索引.md) | B+ 树、聚簇/二级索引、覆盖索引、最左前缀、回表、索引失效 |
| 5 | 🔒 事务与锁 | [5. 事务与锁.md](5.%20事务与锁.md) | ACID、隔离级别、MVCC、ReadView、间隙锁、死锁 |
| 6 | 🔐 锁机制深度 | [6. 锁机制深度.md](6.%20锁机制深度.md) | 意向锁/MDL/AUTO-INC/插入意向锁、兼容矩阵、加锁规则「两原则两优化」、不同 SQL 加锁分析、RC/RR 加锁差异、死锁排查与监控命令、乐观锁 vs 悲观锁 |
| 7 |  explain 深度详解 | [7. explain详解.md](7.%20explain详解.md) | 12 字段全解读、id 执行顺序、key_len 计算公式、Extra 完整速查、多表 JOIN 驱动表选择、format=json / optimizer_trace |
| 8 | 🚀 SQL 优化与调优 | [8. SQL优化与调优.md](8.%20SQL优化与调优.md) | explain、慢查询、深分页、count、join、分库分表、主从复制 |
| 9 | 📄 深分页优化 | [9. 深分页优化.md](9.%20深分页优化.md) | offset 慢因剖析、游标/书签法（多列排序变体）、延迟关联、业务限深、ES/分库分表场景 |
| 10 | 🔗 笛卡尔积 | [10. 笛卡尔积.md](10.%20笛卡尔积.md) | 意外产生场景、CROSS JOIN 正经用途、JOIN 语义=笛卡尔积+过滤 vs 物理执行、JOIN 放大分页/COUNT（先收敛再放大） |
| 11 | 🩺 SQL 优化实际场景 | [11. SQL优化的实际场景.md](11.%20SQL优化的实际场景.md) | 排查五步骨架、索引失效场景、时快时慢分诊、批量导入、CPU 100% 定位、锁超时排查、动态筛选索引设计、慢 SQL 治理 |
...
- **一条 SQL 全链路**：客户端→连接器（认证权限）→分析器（词法/语法建树）→预处理器（语义/权限二次校验）→优化器（Cost 选索引/join 顺序→执行计划）→执行器（handler API 逐行）→InnoDB Buffer Pool（命中返回/未命中读盘）；写操作加锁+改脏页+undo+**redo prepare→binlog→redo commit 两阶段提交**+后台刷脏
- **优化器 Cost 决策**：为什么有索引不走？全表扫 Cost 更低（命中多）/统计信息不准/隐式转换；JOIN 小表驱动大表，被驱动表走索引
