...
- [[mysql/SQL优化的实际场景]]（排查五步骨架/索引失效场景/时快时慢分诊/批量导入/锁超时排查/慢 SQL 治理）
- [[mysql/深分页优化]]
...
为什么 B+树不用 B 树/红黑树 → 聚簇 vs 二级索引 → 回表/索引下推 → MVCC 实现链 → 间隙锁解决幻读 → explain 字段逐个说 → 深分页三种方案 → 分片键怎么选 → 全局 ID（见 [[系统设计/ID生成器]]）→ 线上 SQL 变慢/锁超时怎么排查（场景题，见 [[mysql/SQL优化的实际场景]]）
